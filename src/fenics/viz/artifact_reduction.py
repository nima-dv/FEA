...
def main() -> None:
    ...
    fig = plt.figure(figsize=(13.2, 3.7), dpi=150)
    gs = fig.add_gridspec(1, 3, width_ratios=[1.0, 1.0, 1.18], wspace=0.38,
                          left=0.05, right=0.975, bottom=0.20, top=0.78)

    # Two heat maps: where we started, and where we ended. The middle arm is carried by the
    # curve instead - three near-identical heat maps only waste width.
    for i, p in enumerate((panels[0], panels[-1])):
        ...
        ax.set_title(p["label"] if i == 0 else "All treatments applied", fontsize=11)
        ax.set_xlabel("x [mm]", fontsize=9)
        ax.set_ylim(14, 32)
        ax.tick_params(labelsize=8)
        if i == 0:
            ax.set_ylabel("depth z [mm]", fontsize=9)
        else:
            ax.set_yticklabels([])
            cb = fig.colorbar(im, ax=ax, fraction=0.040, pad=0.045)
            cb.set_label("dB re crack peak", fontsize=7.5)
    ...
    for p, style in zip(panels, ["-", "--", "-"]):
        ...
        has_wall = p["in_wall"].any(axis=1)
        keep = has_wall & (np.abs(p["x"] - p["t_x"]) > 3.0)
    ...
    ax.set_title("Clutter across the wall, crack excluded", fontsize=11)
    ...
    fig.suptitle("Artifact reduction at +20 deg - one shared scale, each panel referenced to "
                 "its own crack peak", fontsize=11.5, y=0.945)
