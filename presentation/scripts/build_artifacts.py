...
IMG = {
    # Was viz/wavefield_web.gif, which was never tracked and did not survive the move to
    # presentation/. wavefield_pair_base.gif is the SAME animation from the same run -
    # the baseline member of the domain-width pair - just rendered at a smaller stride
    # and palette. Using it here costs nothing and removes a dependency on a file that
    # cannot be rebuilt without re-running a snapshot solve.
    "gif": "viz/wavefield_pair_base.gif",
    ...
    # Purpose-built by presentation/scripts/artifact_reduction.py. The stock comparison figures CANNOT show
